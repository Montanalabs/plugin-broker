...
main.os           import router; route()
  ...
  installer.os          feature slice: closed type + quarantined { extract } + using provision { commit }
  runner.os          feature slice: closed type + quarantined { extract } + using exec { commit }
...
- `installer.os` / `runner.os` — the feature slices. `router.os` — injection-safe dispatch.
